...
PATCHED 2026-05-03 (Round 26):
  Adições críticas vs versão original:
    1. "think": False no payload Ollama (desliga thinking blocks)
    2. Image resize com PIL para max edge 1288 + JPEG q=90

  Sem (1): qwen3.5:9b emite ~3× mais tokens, falha JSON em 47% das fotos
  Sem (2): kanbans grandes (>2MB) causam JSON corruption em ~25% dos casos
           porque qwen3.5:9b dropa vírgulas em outputs >6000 chars

  Com ambos: 19/19 photos parseiam, ~22s/photo média.
...
from PIL import Image  # NEW: precisa Pillow

OLLAMA_URL = "http://localhost:11434/api/generate"
MODEL      = "qwen3.5:9b"

# NEW: Resize constraints
_STRIDE = 28                  # Qwen2.5-VL merger requires multiples of 28
_MAX_IMAGE_LONG_EDGE = 1288   # 46 * 28; close to Qwen's 1280 hint

# ── Prompt (versão curta original — funciona bem com resize) ──────────────────
...
- PRI values can be: numbers (1,2,5), codes (c7, C16, C24), or combinations (REP. C9)
- CLIENTE may have no spaces (MTGBELUX) or have spaces (MTG GMBH, DAV NORDIC, LE HAVRE)
- CONI can be a number (10, 12) or text (T, OCT, TORRES)
- If a field is empty or not visible, use ""
...
def _round_to_stride(value):
    rounded = ((value + _STRIDE // 2) // _STRIDE) * _STRIDE
    return max(_STRIDE, rounded)


def image_to_base64(path):
    """NEW: resize to multiples of 28 (Qwen merger constraint) + re-encode JPEG q=90.
    Reduces token count and avoids the comma-drop bug on long sheets."""
...
def main():
    ...
    print(f"  Resize edge: {_MAX_IMAGE_LONG_EDGE}px (PIL LANCZOS, stride-28)")
