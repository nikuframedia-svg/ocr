...
def _empty_refs() -> dict[str, Any]:
    ...
    return {
        ...
        "sap_mtime": 0.0,
        "plan_mtime": 0.0,
        "stats": {"n_lotes": 0, "n_ofs": 0, "n_plan_rows": 0, "n_clientes": 0},
        "available": False,
    }


def _mine_from_excel(sap_path: Path, plan_path: Path) -> dict[str, Any]:
    """Re-mine StockSAP + plan_colunas Excel files into in-memory refs.
    Mirror of scripts/mine_sap_plan.py logic (kept inline to avoid
    ...
    refs["available"] = bool(refs["lotes_sap"] or refs["ofs_plan_str"])
    return refs
...
class RefWatcher:
    ...
    def _needs_reload(self) -> bool:
        sap_m, plan_m = self._current_mtimes()
        return (
    ...
    def get_refs(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            if self._needs_reload():
                ...
                self._refs = _mine_from_excel(self.sap_path, self.plan_path)
